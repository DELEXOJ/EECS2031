...
/************  array of char pointers, each pointer points to an char array ***********/
 ...
 /***** print the substring of pointee of the 5th element pointer (i.e., string
  * "iter"), using both array index notation and pointer notation (already done
  * for you) */
 ...
 /*********finally, access characters in the pointee array *************************/
  ...
  printf("%c  %c\n", *(*(planets + 0) + 0),*(*(pp + 0) + 0)); //  *( *(planets+0) +0)  *(
  // *(pp+0) + 0)  //  print M
  printf("%c  %c\n", *(*(planets + 4) + 03), *(*(pp + 4) + 3));
  // *(*(pp+4)+3) );   // print i
  printf("%c  %c\n", *(*(planets + 6) + 0), *(*(pp + 6) + 0));
